...
void CMatchHistory::OnCreate()
{
    ...
    {
        m_DataFields[i].LoadBackgroundTexture(texture_path,data_field_source_width,data_field_source_height);
        m_DataFields[i].SetSpaceBetweenText(115);
        m_DataFields[i].AddContent(DataModel(std::to_string(m_FieldsSize - i),20,data_color));
        m_DataFields[i].AddContent(DataModel(std::to_string(m_ScoreQueue.front()),20,data_color));
        ...
        int minutes = (int)m_InGameTime.front();
        std::string minutes_seconds = std::to_string(minutes) + " : " + std::to_string(seconds);
        m_DataFields[i].AddContent(DataModel(minutes_seconds,20,data_color,50,20));
        ...
        m_DataFields[i].SetBackgroundPosition(datafield_rect_pos);
        m_DataFields[i].SetBackgroundSize(datafield_rect_size);
